...
export default function DoozyNextPickup() {
  ...
  // ---------------- Memoized pickup logic ----------------
  const { nextPickupDate, showNeutraliser, isPremium, isArtificialGrass } = useMemo(() => {
    if (!subscription?.planStart || !subscription?.plan) {
      return { nextPickupDate: null, showNeutraliser: false, isPremium: false };
    }
    ...
    const isSoilNeutraliser = (planStartTs: number, date: Date) => {
      const start = new Date(planStartTs * 1000);
      const weeksSinceStart = Math.floor((date.getTime() - start.getTime()) / (1000 * 60 * 60 * 24 * 7));
    ...
    switch (plan) {
      ...
      case 'Once a week': {
        next = planDay === 'mon' ? getNextWeekday(today, 1) : getNextWeekday(today, 3);
        break;
      }
      case 'Once a week Artificial Grass': {
        next = getNextWeekday(today, 3); // always Wednesday
        break;
      }
      default:
        next = null;
    ...
    const neutraliser = premium && next ? isSoilNeutraliser(subscription.planStart, next) : false;

    return { nextPickupDate: next, showNeutraliser: neutraliser, isPremium: premium, isArtificialGrass };
  }, [subscription]);
