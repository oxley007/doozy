...
import EditOverrides from './EditOverrides';
...
export default function UserNextSixPickups() {
  ...
  // ---------------- Firestore live listener ----------------
   useEffect(() => {
     ...
     const unsubscribe = firestore()
       ...
       .onSnapshot((docSnap) => {
         ...
         setUserFromFirestore(data);           // full user
   ...
   // ---- REMOVE THIS BROKEN EFFECT (it referenced `nextPickup` which doesn't exist)
   // useEffect(() => {
   //   if (!userId || !subscription) return;
   //   resetExpiredOverrides(userId, subscription, () => {}, new Date(), nextPickup?.date);
   // }, [userId, subscription, nextPickup]);
     ...
     (async () => {
       try {
         ...
         pickups = pickups.map(p => {
           const planStartSeconds =
             typeof p.planStartTs === "number"
               ? p.planStartTs
               : Math.floor(new Date(p.planStart).getTime() / 1000);

           const dateObj = p.date instanceof Date ? p.date : new Date(p.date);

           const features = getPickupFeatures(subscription.plan, planStartSeconds, dateObj);
