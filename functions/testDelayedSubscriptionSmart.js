...
(async () => {
  try {
    ...
    // 2️⃣ Verify token UID matches
    const decoded = await admin.auth().verifyIdToken(FIREBASE_ID_TOKEN);
    console.log('ID token UID:', decoded.uid);

    // 3️⃣ Determine what to do based on subscription
    if (userData?.subscription?.status) {
      console.log(
        'ℹ️ User already has a subscription. Opening setup intent to update payment method...',
      );
      await callEndpoint('create-setup-intent', {}); // send empty JSON body
    } else {
      console.log(
      ...
      await callEndpoint('create-delayed-subscription', body);
    }
  } catch (err) {
